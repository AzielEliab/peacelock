...
Local UI labels match catalog: Open / Seal / Break / Show / Verify / Health / Skill. Upload hashes file bytes and stamps timestamp + date. Worker homepage Live Nodes strip polls `GET /v1/mesh` (default OFF). CLI `peacelock doctor` remains a local self-check — not a FragGate live op.
