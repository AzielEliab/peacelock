...
/**
 * PeaceLock download tracker (Cloudflare Worker).
 *
...
const PROJECT = "peacelock";
const DEFAULT_ASSET = "peacelock-0.1.0.tar.gz";
const DEFAULT_OWNER = "AzielEliab";
const DEFAULT_REPO = "peacelock";
const DEFAULT_BRANCH = "main";
const GITHUB_RELEASES = "https://github.com/AzielEliab/peacelock/releases";
...
async function githubStats(env) {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 PeaceLock-download-tracker", Accept: "application/vnd.github+json" };
  let stars = 0;
  let forks = 0;
  let watchers = 0;
  let release_download_count = 0;
  try {
    const repoRes = await fetch("https://api.github.com/repos/AzielEliab/peacelock", { headers });
    if (repoRes.ok) {
    ...
    const relRes = await fetch("https://api.github.com/repos/AzielEliab/peacelock/releases/latest", { headers });
    if (relRes.ok) {
...
# PeaceLock one-click install. Counted download via this Worker.
set -euo pipefail
HOST="${HOST}"
ASSET="${DEFAULT_ASSET}"
